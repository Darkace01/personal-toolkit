...
function Base64Page() {
  ...
  const encode = () =>
    run(() => (mode === 'url' ? encodeBase64Url(input) : encodeBase64(input)));
  const decode = () =>
    run(() => (mode === 'url' ? decodeBase64Url(input) : decodeBase64(input)));
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <p className="text-gray-400 mb-8">Encode and decode Base64 strings, including URL-safe variant.</p>
      ...
      <div className="space-y-4 mb-4">
        <div>
          ...
          {input && (
            <div className="mt-1 text-xs text-gray-500">
              Valid Base64: {isValid ? <span className="text-green-400">yes</span> : <span className="text-gray-600">no</span>}
            </div>
          )}
        </div>
        ...
        {(output || error) && (
          <div>
            ...
            />
            {output && (
              <button onClick={copy} className="mt-1 text-xs text-amber-400 hover:text-amber-300">
